...
export const userApi = {
  ...
  create: async (data: { username: string; password: string; display_name: string; role: string }) => {
    const res = await client.post('/api/v1/users', data);
    return res.data as User;
  },
  update: async (id: number, data: { display_name: string; role: string; is_active: boolean }) => {
